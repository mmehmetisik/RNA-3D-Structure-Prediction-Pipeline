# RNA 3D Folding - Yapılar Arası Korelasyon Stratejisi
import pandas as pd
import numpy as np
...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (Yapılar Arası Korelasyon Stratejisi)...")
...
# 5. TAHMİN OLUŞTURMA VE YAPILAR ARASI KORELASYON STRATEJİSİ
...
# YAPILAR ARASI KORELASYON STRATEJİSİ
def generate_structures_correlation_strategy(prediction):
    """Yapılar arası korelasyon kullanan geliştirilmiş varyasyon stratejisi"""
    result = []
    ...
    for entry in result:
        ...
        residue_factors[resid] = {
            # Her rezidü için atom korelasyon faktörleri
            'atom_corr_x': np.random.normal(0, 1.0),
            'atom_corr_y': np.random.normal(0, 1.0),
            'atom_corr_z': np.random.normal(0, 1.0),

            # Her rezidü için yapılar arası korelasyon faktörleri
            'struct_2_3': np.random.normal(0, 1.0),  # Yapı 2 ve 3 arasındaki korelasyon
            'struct_3_4': np.random.normal(0, 1.0),  # Yapı 3 ve 4 arasındaki korelasyon
            'struct_4_5': np.random.normal(0, 1.0)  # Yapı 4 ve 5 arasındaki korelasyon
        }

    # Yapı 2: Laplace dağılımı ile ilk yapı
    np.random.seed(123)
    ...
    for entry in result:
        ...
        atom_corr_x = residue_factors[resid]['atom_corr_x']
        atom_corr_y = residue_factors[resid]['atom_corr_y']
        atom_corr_z = residue_factors[resid]['atom_corr_z']

        corr_weight = 0.7  # Korelasyon ağırlığı
        indiv_weight = 0.6  # Bireysel hareket ağırlığı

        # Laplace dağılımı ile korelasyonlu gürültü
        entry['x_2'] = entry['x_1'] + atom_corr_x * noise * corr_weight + np.random.laplace(0, noise * indiv_weight)
        entry['y_2'] = entry['y_1'] + atom_corr_y * noise * corr_weight + np.random.laplace(0, noise * indiv_weight)
        entry['z_2'] = entry['z_1'] + atom_corr_z * noise * corr_weight + np.random.laplace(0, noise * indiv_weight)

    # Yapı 3: Yapı 2'den etkilenen orta seviye korelasyonlu gürültü
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Atom korelasyon faktörleri
        atom_corr_x = residue_factors[resid]['atom_corr_x']
        atom_corr_y = residue_factors[resid]['atom_corr_y']
        atom_corr_z = residue_factors[resid]['atom_corr_z']

        # Yapılar arası korelasyon faktörü
        struct_corr = residue_factors[resid]['struct_2_3']

        # Korelasyon ağırlıkları
        atom_corr_weight = 0.7  # Atom korelasyonu ağırlığı
        struct_corr_weight = 0.3  # Yapılar arası korelasyon ağırlığı
        indiv_weight = 0.5  # Bireysel hareket ağırlığı

        # Yapı 2'den gelen etki
        x2_influence = (entry['x_2'] - entry['x_1']) * struct_corr_weight * struct_corr
        y2_influence = (entry['y_2'] - entry['y_1']) * struct_corr_weight * struct_corr
        z2_influence = (entry['z_2'] - entry['z_1']) * struct_corr_weight * struct_corr

        # Normal dağılım ile korelasyonlu gürültü + yapılar arası korelasyon
        entry['x_3'] = entry['x_1'] + atom_corr_x * noise * atom_corr_weight + x2_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['y_3'] = entry['y_1'] + atom_corr_y * noise * atom_corr_weight + y2_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['z_3'] = entry['z_1'] + atom_corr_z * noise * atom_corr_weight + z2_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)

    # Yapı 4: Yapı 3'ten etkilenen yüksek seviye korelasyonlu gürültü
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Yapılar arası korelasyon faktörü
        struct_corr = residue_factors[resid]['struct_3_4']

        # Korelasyon ağırlıkları
        atom_corr_weight = 0.75  # Daha yüksek atom korelasyonu
        struct_corr_weight = 0.35  # Daha yüksek yapılar arası korelasyon
        indiv_weight = 0.45  # Daha düşük bireysel hareket

        # Yapı 3'ten gelen etki
        x3_influence = (entry['x_3'] - entry['x_1']) * struct_corr_weight * struct_corr
        y3_influence = (entry['y_3'] - entry['y_1']) * struct_corr_weight * struct_corr
        z3_influence = (entry['z_3'] - entry['z_1']) * struct_corr_weight * struct_corr

        # Normal dağılım ile korelasyonlu gürültü + yapılar arası korelasyon
        entry['x_4'] = entry['x_1'] + atom_corr_x * noise * atom_corr_weight + x3_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['y_4'] = entry['y_1'] + atom_corr_y * noise * atom_corr_weight + y3_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['z_4'] = entry['z_1'] + atom_corr_z * noise * atom_corr_weight + z3_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)

    # Yapı 5: Yapı 4'ten etkilenen ultra korelasyonlu gürültü + RNA uçlarına özel odak
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Yapılar arası korelasyon faktörü
        struct_corr = residue_factors[resid]['struct_4_5']

        # Korelasyon ağırlıkları
        atom_corr_weight = 0.8  # En yüksek atom korelasyonu
        struct_corr_weight = 0.4  # En yüksek yapılar arası korelasyon
        indiv_weight = 0.4  # En düşük bireysel hareket

        # Yapı 4'ten gelen etki
        x4_influence = (entry['x_4'] - entry['x_1']) * struct_corr_weight * struct_corr
        y4_influence = (entry['y_4'] - entry['y_1']) * struct_corr_weight * struct_corr
        z4_influence = (entry['z_4'] - entry['z_1']) * struct_corr_weight * struct_corr

        # Normal dağılım ile korelasyonlu gürültü + yapılar arası korelasyon
        entry['x_5'] = entry['x_1'] + atom_corr_x * noise * atom_corr_weight + x4_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['y_5'] = entry['y_1'] + atom_corr_y * noise * atom_corr_weight + y4_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
        entry['z_5'] = entry['z_1'] + atom_corr_z * noise * atom_corr_weight + z4_influence + np.random.normal(0,
                                                                                                               noise * indiv_weight)
...
print("Yapılar Arası Korelasyon stratejisi uygulanıyor...")
all_predictions = []

for prediction in tqdm(base_predictions, desc="RNA Yapıları İşleniyor"):
    structures = generate_structures_correlation_strategy(prediction)
...
strategies = [
    ...
    "Yapı 3: Yapı 2'den Etkilenen Orta Gürültü",
    "Yapı 4: Yapı 3'ten Etkilenen Yüksek Gürültü",
    "Yapı 5: Yapı 4'ten Etkilenen Ultra Gürültü"
]
...
plt.tight_layout()
plt.savefig('structures_correlation_strategy.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nYapılar Arası Korelasyon Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")