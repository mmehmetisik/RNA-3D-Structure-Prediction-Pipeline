# RNA 3D Folding - RNA Motif-Bazlı Varyasyon Stratejisi
import pandas as pd
import numpy as np
...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (RNA Motif-Bazlı Varyasyon Stratejisi)...")
...
# 5. TAHMİN OLUŞTURMA VE RNA MOTİF-BAZLI VARYASYON STRATEJİSİ
print("\n--- TEST TAHMİNLERİ OLUŞTURULUYOR ---")


# RNA motiflerini tahmin etme fonksiyonu
def predict_motifs(sequence):
    """RNA dizisinin olası motiflerini tahmin eder"""
    motifs = []

    # Dizinin uzunluğu
    length = len(sequence)

    # Her nükleotid için motif tahmin et
    for i in range(length):
        # Uç bölgeler (terminal)
        if i < 10 or i > length - 10:
            motifs.append('terminal')
            continue

        # Potansiyel stem bölgesi (GC zengin)
        window = sequence[max(0, i - 2):min(length, i + 3)]  # 5 nükleotidlik pencere
        gc_content = (window.count('G') + window.count('C')) / len(window)

        if gc_content > 0.6:
            motifs.append('stem')
        else:
            # GU wobble çiftleri kontrolü
            if sequence[i] in ['G', 'U'] and i + 10 < length and sequence[i + 10] in ['G', 'U']:
                motifs.append('stem')  # Muhtemel wobble çiftleri stem oluşturabilir
            # Tekrarlı paternler (hairpin loop)
            elif i + 4 < length and sequence[i:i + 3] == sequence[length - i - 3:length - i]:
                motifs.append('hairpin')
            # Potansiyel bulge bölgeleri
            elif sequence[i] == 'A' and (i > 0 and i < length - 1) and sequence[i - 1] != 'A' and sequence[
                i + 1] != 'A':
                motifs.append('bulge')
            # Varsayılan olarak loop
            else:
                motifs.append('loop')
...
def predict_base_structure(test_features, test_sequences):
    ...
    for idx, test_row in test_features.iterrows():
        ...
        # Bu dizinin motiflerini tahmin et
        motifs = predict_motifs(sequence)
        ...
        for i, nucleotide in enumerate(sequence):
            ...
            coords.append({
                ...
                'resname': nucleotide,
                'resid': resid,
                'motif': motifs[i] if i < len(motifs) else 'loop',  # Motif bilgisini ekle
                'x': x_pred,
                'y': y_pred,
                'z': z_pred
            })
...
# RNA MOTİF-BAZLI VARYASYON STRATEJİSİ
def generate_motif_based_strategy(prediction):
    """RNA motiflerine dayalı varyasyon stratejisi"""
    result = []
    ...
    # RNA molekülündeki her nükleotid için korelasyon faktörleri
    np.random.seed(123)
    residue_factors = {}
    ...
    # Yapı 2: Motif-bazlı düşük gürültü
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Adaptif faktörler - motife göre özelleştirilmiş
        if entry['motif'] == 'stem':
            motif_factor = 0.7  # Stem bölgeleri daha sabit
        elif entry['motif'] == 'loop':
            motif_factor = 1.3  # Loop bölgeleri daha esnek
        elif entry['motif'] == 'bulge':
            motif_factor = 1.5  # Bulge bölgeleri çok esnek
        elif entry['motif'] == 'hairpin':
            motif_factor = 1.4  # Hairpin bölgeleri esnek
        else:  # terminal veya diğer
            motif_factor = 1.8  # Uç bölgeleri en esnek

        # Diğer adaptif faktörler
        base_factor = 1.3 if entry['resname'] in ['A', 'U'] else 0.9
        length_factor = max(0.8, min(1.5, 40 / sequence_length))

        # Gürültü seviyesi - motife göre ayarlanmış
        noise = noise_scale_2 * motif_factor * base_factor * length_factor

        # Korelasyonlu gürültü
        corr_factor = residue_factors[resid]['common_x']
        corr_weight = 0.7  # Korelasyon ağırlığı
    ...
    # Yapı 3: Motif-bazlı orta seviye gürültü
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Motife göre özelleştirilmiş faktörler - biraz daha agresif
        if entry['motif'] == 'stem':
            motif_factor = 0.65  # Stem bölgeleri çok daha sabit
        elif entry['motif'] == 'loop':
            motif_factor = 1.4  # Loop bölgeleri daha esnek
        elif entry['motif'] == 'bulge':
            motif_factor = 1.6  # Bulge bölgeleri çok esnek
        elif entry['motif'] == 'hairpin':
            motif_factor = 1.5  # Hairpin bölgeleri esnek
        else:  # terminal veya diğer
            motif_factor = 2.0  # Uç bölgeleri çok daha esnek
        ...
        # Gürültü seviyesi
        noise = noise_scale_3 * motif_factor * base_factor * length_factor

        # Korelasyonlu gürültü
        corr_factor = residue_factors[resid]['common_y']
        corr_weight = 0.7
        indiv_weight = 0.6
    ...
    # Yapı 4: Motif-bazlı yüksek seviye gürültü
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Motife göre özelleştirilmiş faktörler - daha da agresif
        if entry['motif'] == 'stem':
            motif_factor = 0.6  # Stem bölgeleri çok çok sabit
        elif entry['motif'] == 'loop':
            motif_factor = 1.5  # Loop bölgeleri çok daha esnek
        elif entry['motif'] == 'bulge':
            motif_factor = 1.8  # Bulge bölgeleri aşırı esnek
        elif entry['motif'] == 'hairpin':
            motif_factor = 1.7  # Hairpin bölgeleri çok esnek
        else:  # terminal veya diğer
            motif_factor = 2.3  # Uç bölgeleri ultra esnek
        ...
        # Gürültü seviyesi
        noise = noise_scale_4 * motif_factor * base_factor * length_factor

        # Korelasyonlu gürültü
        corr_factor = residue_factors[resid]['common_z']
        corr_weight = 0.75  # Daha yüksek korelasyon
    ...
    # Yapı 5: Motif-bazlı ultra seviye gürültü + Koordineli motif hareketi
    np.random.seed(123)
    noise_scale_5 = 0.70 * 10

    # Önce her motif tipi için ortak faktörler oluştur
    # Bu, aynı motif tipindeki nükleotidlerin benzer şekilde hareket etmesini sağlar
    motif_common_factors = {
        'stem': {'x': np.random.normal(0, 1.0), 'y': np.random.normal(0, 1.0), 'z': np.random.normal(0, 1.0)},
        'loop': {'x': np.random.normal(0, 1.0), 'y': np.random.normal(0, 1.0), 'z': np.random.normal(0, 1.0)},
        'bulge': {'x': np.random.normal(0, 1.0), 'y': np.random.normal(0, 1.0), 'z': np.random.normal(0, 1.0)},
        'hairpin': {'x': np.random.normal(0, 1.0), 'y': np.random.normal(0, 1.0), 'z': np.random.normal(0, 1.0)},
        'terminal': {'x': np.random.normal(0, 1.0), 'y': np.random.normal(0, 1.0), 'z': np.random.normal(0, 1.0)}
    }
    ...
    for entry in result:
        ...
        # Motife göre en agresif faktörler
        if motif == 'stem':
            motif_factor = 0.55  # Stem bölgeleri ultra sabit
        elif motif == 'loop':
            motif_factor = 1.7  # Loop bölgeleri ultra esnek
        elif motif == 'bulge':
            motif_factor = 2.0  # Bulge bölgeleri aşırı esnek
        elif motif == 'hairpin':
            motif_factor = 1.8  # Hairpin bölgeleri çok esnek
        else:  # terminal veya diğer
            motif_factor = 2.5  # Uç bölgeleri ekstrem esnek

        # Diğer adaptif faktörler
        base_factor = 1.3 if entry['resname'] in ['A', 'U'] else 0.8
        length_factor = max(0.7, min(1.8, 35 / sequence_length))

        # Gürültü seviyesi
        noise = noise_scale_5 * motif_factor * base_factor * length_factor

        # Motif-bazlı koordineli hareket
        # Aynı motif tipindeki tüm nükleotidler benzer şekilde hareket eder
        motif_corr_x = motif_common_factors[motif]['x'] if motif in motif_common_factors else np.random.normal(0, 1.0)
        motif_corr_y = motif_common_factors[motif]['y'] if motif in motif_common_factors else np.random.normal(0, 1.0)
        motif_corr_z = motif_common_factors[motif]['z'] if motif in motif_common_factors else np.random.normal(0, 1.0)

        # Rezidü-spesifik korelasyon + motif-bazlı korelasyon
        corr_factor = residue_factors[resid]['common_x'] * 0.3 + motif_corr_x * 0.7
    ...
    # Motif bilgisini submission'dan temizle
    for entry in result:
        entry.pop('motif', None)

    return result


# Her RNA için Motif-Bazlı Varyasyon stratejisi uygula
print("RNA Motif-Bazlı Varyasyon stratejisi uygulanıyor...")
all_predictions = []

for prediction in tqdm(base_predictions, desc="RNA Yapıları İşleniyor"):
    structures = generate_motif_based_strategy(prediction)
...
strategies = [
    ...
    "Yapı 2: Motif-Bazlı Düşük Gürültü",
    "Yapı 3: Motif-Bazlı Orta Gürültü",
    "Yapı 4: Motif-Bazlı Yüksek Gürültü",
    "Yapı 5: Motif-Bazlı Ultra Gürültü"
]
...
plt.tight_layout()
plt.savefig('motif_based_strategy.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nRNA Motif-Bazlı Varyasyon Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")