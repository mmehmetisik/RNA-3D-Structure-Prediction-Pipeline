# RNA 3D Folding - Agresif Gürültü Spektrumu Stratejisi
import pandas as pd
import numpy as np
...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (Agresif Gürültü Spektrumu Stratejisi)...")
...
# 5. TAHMİN OLUŞTURMA VE AGRESİF GÜRÜLTÜ SPEKTRUMU STRATEJİSİ
...
# AGRESİF GÜRÜLTÜ SPEKTRUMU STRATEJİSİ
def generate_aggressive_noise_strategy(prediction):
    """Çok daha agresif gürültü spektrumu ve çeşitli dağılımlar"""
    result = []
    ...
    # ULTRA-AGRESİF GÜRÜLTÜ SPEKTRUMU

    # Yapı 2: Orta gürültü - Gaussian (0.40 * 10)
    np.random.seed(123)  # En başarılı tohum değeri
    noise_scale_2 = 0.40 * 10
    ...
    # Yapı 3: Yüksek gürültü - Log-normal dağılım (0.70 * 10)
    np.random.seed(123)
    noise_scale_3 = 0.70 * 10
    for i, entry in enumerate(result):
        # Log-normal dağılım (pozitif yönde daha fazla yayılma)
        x_noise = np.random.lognormal(0, 0.8) * noise_scale_3 * (np.random.rand() > 0.5 and 1 or -1)
        y_noise = np.random.lognormal(0, 0.8) * noise_scale_3 * (np.random.rand() > 0.5 and 1 or -1)
        z_noise = np.random.lognormal(0, 0.8) * noise_scale_3 * (np.random.rand() > 0.5 and 1 or -1)

        entry['x_3'] = entry['x_1'] + x_noise
        entry['y_3'] = entry['y_1'] + y_noise
        entry['z_3'] = entry['z_1'] + z_noise

    # Yapı 4: Ultra yüksek gürültü - Gaussian (1.10 * 10)
    np.random.seed(123)
    noise_scale_4 = 1.10 * 10  # Çok daha agresif!
    for i, entry in enumerate(result):
    ...
    # Yapı 5: Extreme gürültü - Cauchy dağılım (0.90 * 10)
    # Cauchy dağılımı, uzun kuyruklu bir dağılımdır - çok uzak değerleri daha sık örnekler
    np.random.seed(123)
    noise_scale_5 = 0.90 * 10
    for i, entry in enumerate(result):
        # Cauchy dağılımı
        def cauchy_sample():
            return np.tan(np.pi * (np.random.rand() - 0.5))

        # Pozisyona bağlı faktör - RNA uçlarında daha fazla esneklik
        resid = entry['resid']
        rel_pos = resid / sequence_length
        edge_factor = 2.5 if rel_pos < 0.15 or rel_pos > 0.85 else 1.0

        # Cauchy gürültüsü (uzun kuyruklu, aşırı uç değerler için)
        x_noise = cauchy_sample() * noise_scale_5 * edge_factor * 0.3  # 0.3 ile ölçeklendirme - çok aşırı değerleri engeller
        y_noise = cauchy_sample() * noise_scale_5 * edge_factor * 0.3
        z_noise = cauchy_sample() * noise_scale_5 * edge_factor * 0.3

        # Aşırı uç değerleri kırp
        x_noise = np.clip(x_noise, -5 * noise_scale_5, 5 * noise_scale_5)
        y_noise = np.clip(y_noise, -5 * noise_scale_5, 5 * noise_scale_5)
        z_noise = np.clip(z_noise, -5 * noise_scale_5, 5 * noise_scale_5)

        entry['x_5'] = entry['x_1'] + x_noise
        entry['y_5'] = entry['y_1'] + y_noise
        entry['z_5'] = entry['z_1'] + z_noise

    return result


# Her RNA için Agresif Gürültü Spektrumu stratejisi uygula
print("Agresif Gürültü Spektrumu stratejisi uygulanıyor...")
all_predictions = []

for prediction in base_predictions:
    structures = generate_aggressive_noise_strategy(prediction)
...
strategies = [
    ...
    "Yapı 2: Orta Gürültü - Gaussian (0.40*10)",
    "Yapı 3: Yüksek Gürültü - Log-normal (0.70*10)",
    "Yapı 4: Ultra Gürültü - Gaussian (1.10*10)",
    "Yapı 5: Extreme Gürültü - Cauchy (0.90*10)"
]
...
plt.tight_layout()
plt.savefig('aggressive_noise_spectrum.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nAgresif Gürültü Spektrumu Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")