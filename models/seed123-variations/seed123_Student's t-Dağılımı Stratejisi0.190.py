# RNA 3D Folding - Student's t-Dağılımı Stratejisi
import pandas as pd
import numpy as np
...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (Student's t-Dağılımı Stratejisi)...")
...
# 5. TAHMİN OLUŞTURMA VE STUDENT'S T-DAĞILIMI STRATEJİSİ
...
# STUDENT'S T-DAĞILIMI STRATEJİSİ
def generate_student_t_strategy(prediction):
    """Student's t-dağılımı kullanan geliştirilmiş varyasyon stratejisi"""
    result = []
    ...
    # Student's t dağılımından örnek üretme fonksiyonu
    def student_t_sample(df=3.0, scale=1.0):
        """
        df: degrees of freedom parametresi (küçüldükçe daha ağır kuyruklu)
        scale: ölçeklendirme faktörü
        """
        return np.random.standard_t(df) * scale

    # Yapı 2: Student's t-dağılımı ile düşük gürültü (df=5.0)
    np.random.seed(123)
    ...
    for entry in result:
        ...
        corr_factor = residue_factors[resid]['common_x']  # Aynı faktör X, Y ve Z için
        corr_weight = 0.7  # Korelasyon ağırlığı
        indiv_weight = 0.6  # Bireysel hareket ağırlığı

        # Student's t-dağılımı (df=5.0) - orta ağırlıkta kuyruk
        entry['x_2'] = entry['x_1'] + corr_factor * noise * corr_weight + student_t_sample(df=5.0,
                                                                                           scale=noise * indiv_weight)
        entry['y_2'] = entry['y_1'] + corr_factor * noise * corr_weight + student_t_sample(df=5.0,
                                                                                           scale=noise * indiv_weight)
        entry['z_2'] = entry['z_1'] + corr_factor * noise * corr_weight + student_t_sample(df=5.0,
                                                                                           scale=noise * indiv_weight)

    # Yapı 3: Student's t-dağılımı ile orta-yüksek gürültü (df=4.0)
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Korelasyonlu gürültü
        corr_factor = residue_factors[resid]['common_y']  # Farklı faktör
        corr_weight = 0.7
        indiv_weight = 0.6

        # Student's t-dağılımı (df=4.0) - daha ağır kuyruk
        entry['x_3'] = entry['x_1'] + corr_factor * noise * corr_weight + student_t_sample(df=4.0,
                                                                                           scale=noise * indiv_weight)
        entry['y_3'] = entry['y_1'] + corr_factor * noise * corr_weight + student_t_sample(df=4.0,
                                                                                           scale=noise * indiv_weight)
        entry['z_3'] = entry['z_1'] + corr_factor * noise * corr_weight + student_t_sample(df=4.0,
                                                                                           scale=noise * indiv_weight)

    # Yapı 4: Student's t-dağılımı ile çok yüksek gürültü (df=3.0)
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # Korelasyonlu gürültü
        corr_factor = residue_factors[resid]['common_z']  # Farklı faktör
        corr_weight = 0.75  # Daha yüksek korelasyon
        indiv_weight = 0.55  # Daha düşük bireysel hareket

        # Student's t-dağılımı (df=3.0) - çok ağır kuyruk
        entry['x_4'] = entry['x_1'] + corr_factor * noise * corr_weight + student_t_sample(df=3.0,
                                                                                           scale=noise * indiv_weight)
        entry['y_4'] = entry['y_1'] + corr_factor * noise * corr_weight + student_t_sample(df=3.0,
                                                                                           scale=noise * indiv_weight)
        entry['z_4'] = entry['z_1'] + corr_factor * noise * corr_weight + student_t_sample(df=3.0,
                                                                                           scale=noise * indiv_weight)

    # Yapı 5: Student's t-dağılımı ile ultra gürültü + RNA uçlarına özel odak (df=2.0)
    np.random.seed(123)
    ...
    for entry in result:
        ...
        # En yüksek korelasyonlu gürültü
        # Üç faktörün ortalamasını al (daha dengeli hareket)
        corr_factor = (residue_factors[resid]['common_x'] +
                       residue_factors[resid]['common_y'] +
                       residue_factors[resid]['common_z']) / 3.0

        corr_weight = 0.8  # En yüksek korelasyon
        indiv_weight = 0.5  # En düşük bireysel hareket

        # Student's t-dağılımı (df=2.0) - aşırı ağır kuyruk
        entry['x_5'] = entry['x_1'] + corr_factor * noise * corr_weight + student_t_sample(df=2.0,
                                                                                           scale=noise * indiv_weight)
        entry['y_5'] = entry['y_1'] + corr_factor * noise * corr_weight + student_t_sample(df=2.0,
                                                                                           scale=noise * indiv_weight)
        entry['z_5'] = entry['z_1'] + corr_factor * noise * corr_weight + student_t_sample(df=2.0,
                                                                                           scale=noise * indiv_weight)

    return result


# Her RNA için Student's t-dağılımı stratejisi uygula
print("Student's t-Dağılımı stratejisi uygulanıyor...")
all_predictions = []

for prediction in tqdm(base_predictions, desc="RNA Yapıları İşleniyor"):
    structures = generate_student_t_strategy(prediction)
...
strategies = [
    ...
    "Yapı 2: Student's t (df=5.0) Düşük Gürültü",
    "Yapı 3: Student's t (df=4.0) Orta Gürültü",
    "Yapı 4: Student's t (df=3.0) Yüksek Gürültü",
    "Yapı 5: Student's t (df=2.0) Ultra Gürültü"
]
...
plt.tight_layout()
plt.savefig('student_t_strategy.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nStudent's t-Dağılımı Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")