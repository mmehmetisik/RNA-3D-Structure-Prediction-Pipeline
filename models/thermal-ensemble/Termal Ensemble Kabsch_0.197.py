...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (Termal Ensemble Kabsch)...")
...
# 5. TAHMİN OLUŞTURMA VE TERMAL ENSEMBLE KABSCH STRATEJİSİ
...
# TERMAL ENSEMBLE KABSCH STRATEJİSİ
def generate_thermal_ensemble_kabsch(prediction):
    ...
    # Student's t dağılımı için serbestlik derecesi
    df = 5
    ...
    # Student's t dağılımı için daha düşük serbestlik derecesi (daha ağır kuyruk)
    df = 4
    ...
    for i, entry in enumerate(result):
        ...
        influence_2 = 0.05  # Yapı 2'den etki (düşük sıcaklık)
        influence_3 = 0.10  # Yapı 3'ten etki (orta sıcaklık)
        influence_4 = 0.15  # Yapı 4'ten etki (yüksek sıcaklık)
...
print("Örnek RNA'nın çoklu yapılarını görselleştiriliyor...")
...
plt.tight_layout()
plt.savefig('termal_ensemble_kabsch.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nTermal Ensemble Kabsch Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")