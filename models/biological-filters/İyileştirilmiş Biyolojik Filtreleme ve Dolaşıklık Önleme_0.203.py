...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (İyileştirilmiş Biyolojik Filtreleme ve Dolaşıklık Önleme)...")
...
def generate_thermal_ensemble_kabsch(prediction):
    ...
    # --------------------------------------------------
    # Yapı 2: DÜŞÜK SICAKLIK (0.24 * 10) - Laplace dağılımı - Hafifçe azaltıldı
    # --------------------------------------------------
    np.random.seed(seed_base)
    noise_scale_2 = 0.24 * 10  # 0.25'ten 0.24'e azaltıldı
    ...
    for i, entry in enumerate(result):
        ...
        if nucleotide == 'A':
            base_factor = 1.65  # A en esnek (1.6 -> 1.65)
        elif nucleotide == 'U':
            base_factor = 1.45  # U oldukça esnek (1.4 -> 1.45)
        elif nucleotide == 'G':
            base_factor = 0.73  # G oldukça stabil (0.75 -> 0.73)
        else:  # C
            base_factor = 0.83  # C stabil (0.85 -> 0.83)

        length_factor = max(0.8, min(1.5, 40 / sequence_length))

        # Kenar faktörü
        if rel_pos < 0.15 or rel_pos > 0.85:
            edge_factor = 1.85  # Uçlarda daha esnek (1.8 -> 1.85)
        else:
        ...
        motif = motifs[i]
        if motif == 'stem':
            edge_factor *= 0.68  # Stem bölgeleri daha az hareket eder (0.7 -> 0.68)
        elif motif == 'loop':
            edge_factor *= 1.22  # Loop bölgeleri daha fazla hareket eder (1.2 -> 1.22)
        elif motif == 'bulge':
            edge_factor *= 1.55  # Bulge bölgeleri çok daha fazla hareket eder (1.5 -> 1.55)
        elif motif == 'hairpin':
            edge_factor *= 1.35  # Hairpin bölgeleri daha fazla hareket eder (1.3 -> 1.35)
        elif motif == 'terminal':
            edge_factor *= 1.85  # Terminal bölgeler en fazla hareket eder (1.8 -> 1.85)
        elif motif == 'stem_edge':
            edge_factor *= 0.83  # Stem kenarları orta seviye hareket eder (0.85 -> 0.83)
        ...
        corr_weight = 0.73  # İnce ayarlanmış korelasyon ağırlığı (0.72 -> 0.73)
        indiv_weight = 0.62  # Bireysel gürültü ağırlığı (0.6 -> 0.62)
    ...
    # --------------------------------------------------
    # Yapı 3: ORTA SICAKLIK (0.43 * 10) - Student's t + Normal karışımı dağılımı - Hafifçe azaltıldı
    # --------------------------------------------------
    np.random.seed(seed_base + 1)
    noise_scale_3 = 0.43 * 10  # Orta derecede gürültü (0.45 -> 0.43)
    ...
    for i, entry in enumerate(result):
        ...
        # Adaptif faktörler - İYİLEŞTİRİLMİŞ NÜKLEOTİD FAKTÖRÜ
        if nucleotide == 'A':
            base_factor = 1.65  # (1.6 -> 1.65)
        elif nucleotide == 'U':
            base_factor = 1.45  # (1.4 -> 1.45)
        elif nucleotide == 'G':
            base_factor = 0.73  # (0.75 -> 0.73)
        else:  # C
            base_factor = 0.83  # (0.85 -> 0.83)

        length_factor = max(0.8, min(1.5, 40 / sequence_length))

        # Kenar faktörü - daha agresif
        if rel_pos < 0.15 or rel_pos > 0.85:
            edge_factor = 2.1  # Uçlarda çok daha esnek (2.0 -> 2.1)
        else:
            edge_factor = 1.0
        ...
        corr_weight = 0.75  # İnce ayarlanmış korelasyon ağırlığı (0.74 -> 0.75)
        indiv_weight = 0.62  # Bireysel gürültü ağırlığı (0.6 -> 0.62)
    ...
    # --------------------------------------------------
    # Yapı 4: YÜKSEK SICAKLIK (0.72 * 10) - Student's t dağılımı - Hafifçe azaltıldı
    # --------------------------------------------------
    np.random.seed(seed_base + 2)
    noise_scale_4 = 0.72 * 10  # Yüksek derecede gürültü (0.75 -> 0.72)
    ...
    for i, entry in enumerate(result):
        ...
        # Adaptif faktörler - İYİLEŞTİRİLMİŞ NÜKLEOTİD FAKTÖRÜ
        if nucleotide == 'A':
            base_factor = 1.75  # Yüksek sıcaklıkta daha esnek (1.7 -> 1.75)
        elif nucleotide == 'U':
            base_factor = 1.55  # Yüksek sıcaklıkta daha esnek (1.5 -> 1.55)
        elif nucleotide == 'G':
            base_factor = 0.78  # Nispeten daha stabil (0.8 -> 0.78)
        else:  # C
            base_factor = 0.88  # Nispeten daha stabil (0.9 -> 0.88)

        length_factor = max(0.7, min(1.6, 40 / sequence_length))

        # Kenar faktörü - daha agresif
        if rel_pos < 0.1 or rel_pos > 0.9:
            edge_factor = 2.6  # Uçlarda çok çok daha esnek (2.5 -> 2.6)
        elif rel_pos < 0.2 or rel_pos > 0.8:
            edge_factor = 1.9  # Uçlara yakın daha esnek (1.8 -> 1.9)
        else:
            edge_factor = 1.0
        ...
        corr_weight = 0.78  # İnce ayarlanmış korelasyon ağırlığı (0.77 -> 0.78)
        indiv_weight = 0.55  # Bireysel gürültü ağırlığı - aynı
    ...
    noise_scale_5 = 0.63 * 10  # Orta-yüksek gürültü (0.65 -> 0.63)
    ...
    for i, entry in enumerate(result):
        ...
        # Kenar faktörü - daha agresif
        if rel_pos < 0.1 or rel_pos > 0.9:
            edge_factor = 3.1  # Uçlarda çok çok çok daha esnek (3.0 -> 3.1)
        elif rel_pos < 0.2 or rel_pos > 0.8:
            edge_factor = 2.1  # Uçlara yakın çok daha esnek (2.0 -> 2.1)
        else:
            edge_factor = 1.0
        ...
        corr_weight = 0.84  # İnce ayarlanmış korelasyon ağırlığı (0.82 -> 0.84)
        indiv_weight = 0.5  # Bireysel gürültü ağırlığı - aynı
...
# DOLAŞIKLIK KONTROLÜ - YENİ MODÜL
def check_entanglements(coords, sec_structure):
    ...
    # 1. Heliks bölgelerini belirle
    helix_regions = identify_helix_regions(sec_structure)

    # 2. Her heliks için, diğer nükleotidlerin bu heliksin içinden geçip geçmediğini kontrol et
    for start, end in helix_regions:
        if end - start < 3:
            continue  # Çok kısa heliksleri atla

        # Heliksin merkezini ve normal vektörünü hesapla
        points_in_helix = coords[start:end + 1]
        center = np.mean(points_in_helix, axis=0)

        # Heliksin düzlemini ve normal vektörünü hesapla (kabaca)
        if len(points_in_helix) >= 3:
            ...
            if norm > 0:
                ...
                for i in range(n - 1):
                    if i >= start - 1 and i + 1 <= end + 1:
                        continue  # Heliksin kendisini ve hemen komşu bölgeleri atla
                    ...
                    if side1 * side2 < 0:
                        # Kesme noktasını daha detaylı kontrol et
                        # İki nokta arasındaki doğruyu parametrik olarak ifade et ve
                        # bu doğrunun düzlemle kesiştiği yeri hesapla
                        t = side1 / (side1 - side2)  # Kesişim noktasının parametresi
                        ...
                        # Heliksin yarıçapını yaklaşık olarak hesapla
                        helix_radius = np.mean([np.linalg.norm(p - center) for p in points_in_helix])

                        # Eğer kesişim noktası heliksin içindeyse dolaşıklık var demektir
                        if dist_to_center < helix_radius * 1.2:  # Biraz tolerans ekle
                            entanglements.append({
                                'type': 'helix_penetration',
                                'segment': (i, i + 1),
                                'helix': (start, end),
                                'severity': (helix_radius - dist_to_center) / helix_radius  # 0-1 arası şiddet
                            })

    return entanglements


# İYİLEŞTİRİLMİŞ BİYOLOJİK FİLTRELEME VE DOLAŞIKLIK ÖNLEME MODÜLÜ
def apply_improved_biological_filtering(structures, prediction):
    """İyileştirilmiş biyolojik filtreleme ve dolaşıklık önleme uygular"""
    # RNA dizisi ve motif bilgileri
    sequence = prediction['sequence']
    motifs = prediction['motifs']
    ...
    # Her yapı için atom çakışmalarını kontrol et
    for struct_idx in range(2, 6):  # Yapı 2-5 için
        # Koordinat matrisini oluştur
        coords = np.array([
            [entry[f'x_{struct_idx}'], entry[f'y_{struct_idx}'], entry[f'z_{struct_idx}']]
            for entry in filtered_structures
        ])

        # Atomlar arası mesafeleri hesapla
        n = len(coords)
        min_allowed_dist = 3.4  # Minimum izin verilen atom mesafesi (Å) - daha hassas (3.5 -> 3.4)
        ideal_bond_length = 6.0  # İdeal bağ uzunluğu - RNA omurgası için

        # 1. DOLAŞIKLIK KONTROLÜ VE DÜZELTME - YENİ MODÜL
        # Dolaşıklıkları kontrol et
        entanglements = check_entanglements(coords, sec_structure)

        # Eğer dolaşıklık varsa düzelt
        if entanglements:
            for entanglement in entanglements:
                if entanglement['type'] == 'helix_penetration':
                    ...
                    # Şiddet ne kadar yüksekse o kadar fazla uzaklaştırılır
                    ...
                    # Bu, helisin şeklini korumaya yardımcı olur
                    coords[h_start] -= vec * push_dist * 0.2
                    coords[h_end] -= vec * push_dist * 0.2

        # 2. İteratif düzeltme - İYİLEŞTİRİLMİŞ
        for iteration in range(6):  # Daha fazla iterasyon (5'ten 6'ya çıkarıldı)
            changes_made = False

            # 2.1 Atom çakışmalarını düzelt - kademeli yaklaşım
            correction_strength = 0.85 - (iteration * 0.12)  # İterasyonla azalan düzeltme gücü (0.8-0.15 -> 0.85-0.12)
            ...
            for i in range(n):
                for j in range(i + 2, n):  # Ardışık olmayan atomlar için
                    ...
                    # Çakışma varsa (mesafe çok kısaysa)
                    if dist < min_allowed_dist:
                        changes_made = True

                        # İki atom arasındaki vektör (j'den i'ye doğru)
                        vec = coords[j] - coords[i]
                        if np.linalg.norm(vec) > 0:
                            vec = vec / np.linalg.norm(vec)  # Birim vektör

                        # İtme gücü (mesafe ne kadar yakınsa o kadar güçlü)
                        push_force = (min_allowed_dist - dist) / 2.0 * correction_strength

                        # Motif tiplerine göre düzeltme ağırlıkları - İYİLEŞTİRİLMİŞ
                        weight_i = 0.5
                        weight_j = 0.5
                        ...
                        if motif_i == 'stem':
                            weight_i = 0.28  # (0.3 -> 0.28)
                        elif motif_i in ['loop', 'bulge', 'hairpin']:
                            weight_i = 0.72  # (0.7 -> 0.72)
                        elif motif_i == 'terminal':
                            weight_i = 0.85  # Terminal bölgeler en çok hareket eder (0.8 -> 0.85)

                        if motif_j == 'stem':
                            weight_j = 0.28  # (0.3 -> 0.28)
                        elif motif_j in ['loop', 'bulge', 'hairpin']:
                            weight_j = 0.72  # (0.7 -> 0.72)
                        elif motif_j == 'terminal':
                            weight_j = 0.85  # (0.8 -> 0.85)
            ...
            # 2.2. Ardışık nükleotidler arası bağ uzunluğu kontrolü - İYİLEŞTİRİLMİŞ
            bond_correction_strength = 0.75 - (
                        iteration * 0.1)  # İterasyonla azalan düzeltme gücü (0.7-0.1 -> 0.75-0.1)

            for i in range(n - 1):
                # İki ardışık nükleotid arasındaki mesafe
                dist = np.linalg.norm(coords[i] - coords[i + 1])

                # Eğer bağ uzunluğu çok farklıysa düzelt
                if abs(dist - ideal_bond_length) > 1.1:  # Tolerans 1.2'den 1.1'e azaltıldı
                    changes_made = True
                    ...
                    if dist > ideal_bond_length:
                        correction = (dist - ideal_bond_length) / 2.0 * bond_correction_strength  # Yaklaştır
                    else:
                        correction = (ideal_bond_length - dist) / 2.0 * bond_correction_strength  # Uzaklaştır
            ...
            # 3. Watson-Crick baz çiftleri arası mesafeleri düzelt - İYİLEŞTİRİLMİŞ
            if struct_idx > 2:  # Sadece yapı 3, 4 ve 5'te uygula (Yapı 2'yi daha serbest bırak)
                wc_correction_strength = 0.65 - (iteration * 0.08)  # (0.6-0.1 -> 0.65-0.08)
                ...
                for i, char in enumerate(sec_structure):
                    if char == '(':
                        ...
                        j = stack.pop()
                        wc_pairs.append((j, i))  # (açılış, kapanış) indekslerini kaydet
                ...
                for i, j in wc_pairs:
                    if j - i > 2:  # Gerçek W-C çiftleri en az 3 nükleotid aralıklı olmalı
                        ...
                        # İdeal W-C çifti mesafesi (baz tipine göre daha hassas ayarlanmış)
                        ideal_wc_dist = 10.0  # Varsayılan değer
                        if (sequence[i] == 'G' and sequence[j] == 'C') or (sequence[i] == 'C' and sequence[j] == 'G'):
                            ideal_wc_dist = 10.4  # G-C için biraz daha uzun (10.5 -> 10.4)
                        elif (sequence[i] == 'A' and sequence[j] == 'U') or (sequence[i] == 'U' and sequence[j] == 'A'):
                            ideal_wc_dist = 10.0  # A-U için biraz daha kısa
                        elif (sequence[i] == 'G' and sequence[j] == 'U') or (sequence[i] == 'U' and sequence[j] == 'G'):
                            ideal_wc_dist = 10.2  # G-U wobble için orta mesafe (10.3 -> 10.2)

                        # Eğer mesafe çok farklıysa düzelt
                        if abs(dist - ideal_wc_dist) > 1.8:  # Tolerans 2.0'dan 1.8'e azaltıldı
                            changes_made = True
                            ...
                            if dist > ideal_wc_dist:
                                correction = (
                                                         dist - ideal_wc_dist) * 0.35 * wc_correction_strength  # Yaklaştır (0.3 -> 0.35)
                            else:
                                correction = (
                                                         ideal_wc_dist - dist) * 0.35 * wc_correction_strength  # Uzaklaştır (0.3 -> 0.35)
    ...
    # Biyolojik olarak filtre edilmiş yapıları döndür
...
def process_prediction(prediction):
    ...
    # 1. Temel Termal Ensemble Kabsch stratejisi
    structures = generate_thermal_ensemble_kabsch(prediction)

    # 2. İyileştirilmiş Biyolojik Filtreleme ve Dolaşıklık Önleme
    structures = apply_improved_biological_filtering(structures, prediction)

    return structures


# 7. TAHMİN OLUŞTURMA VE ÇALIŞTIRMA
...
print("İyileştirilmiş Biyolojik Filtreleme ve Dolaşıklık Önleme stratejisi uygulanıyor...")
all_predictions = []
...
strategies = [
    ...
    "Yapı 2: Düşük Sıcaklık (0.24*10) - Laplace",
    "Yapı 3: Orta Sıcaklık (0.43*10) - Student's t + Normal",
    "Yapı 4: Yüksek Sıcaklık (0.72*10) - Student's t",
    "Yapı 5: Kabsch Hibrit (0.63*10) - Karma"
]
...
plt.tight_layout()
plt.savefig('improved_with_entanglement_prevention.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(
    f"\nİyileştirilmiş Biyolojik Filtreleme ve Dolaşıklık Önleme Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")

# İyileştirmeleri özetle
print("\n--- İYİLEŞTİRME ÖZETİ ---")
print("1. Biyolojik Filtreleme - Optimize Edilmiş:")
print("   - Daha fazla iterasyon (5'ten 6'ya) ve iterasyonla azalan daha hassas düzeltme gücü")
print("   - Atom çakışmaları için daha hassas minimum mesafe (3.5Å -> 3.4Å)")
print("   - Bağ uzunlukları için daha sıkı tolerans (1.2Å -> 1.1Å)")
print("   - Watson-Crick baz çiftleri için ince ayarlanmış mesafeler (GC: 10.4Å, AU: 10.0Å, GU: 10.2Å)")
print("   - Terminal bölgeler için yükseltilmiş hareketlilik faktörü (0.8 -> 0.85)")
print("   - Stem bölgeleri için azaltılmış hareketlilik faktörü (0.3 -> 0.28)")

print("\n2. Dolaşıklık Önleme - Yeni Modül:")
print("   - Heliksler arasında dolaşıklık tespiti")
print("   - Dolaşıklık şiddetine göre adaptif düzeltme")
print("   - Dolaşıklığı önlemek için RNA segmentlerini akıllı şekilde yeniden konumlandırma")

print("\n3. Termal Ensemble Parametreleri - İnce Ayarlanmış:")
print("   - Sıcaklık seviyelerinde optimizasyon (0.25->0.24, 0.45->0.43, 0.75->0.72, 0.65->0.63)")
print("   - Nükleotid faktörleri ince ayarlandı (A: 1.65, U: 1.45, G: 0.73, C: 0.83)")
print("   - Korelasyon ağırlıkları ve bireysel gürültü parametreleri optimize edildi")

print("\nBu iyileştirmeler, Nature Methods makalesinde vurgulanan prensipleri daha iyi uygulamayı hedeflemektedir:")
print("- Watson-Crick baz çiftlerinin doğru konumlandırılması")
print("- Dolaşıklıkların (entanglements) önlenmesi")
print("- Helikal bölgelerin doğru korunması")