...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (İyileştirilmiş Biyolojik Filtreleme ile Termal Ensemble Kabsch)...")
...
# İYİLEŞTİRİLMİŞ BİYOLOJİK FİLTRELEME MODÜLÜ
def apply_improved_biological_filtering(structures, prediction):
    """İyileştirilmiş biyolojik filtreleme uygular - gelişmiş atom çakışması ve bağ uzunluğu düzeltmesi"""
    ...
    for struct_idx in range(2, 6):  # Yapı 2-5 için
        ...
        n = len(coords)
        min_allowed_dist = 3.5  # Minimum izin verilen atom mesafesi (Å) - hafifçe azaltıldı
        ideal_bond_length = 6.0  # İdeal bağ uzunluğu - RNA omurgası için

        # İteratif düzeltme
        for iteration in range(5):  # Daha fazla iterasyon (3'ten 5'e çıkarıldı)
            changes_made = False

            # 1. Atom çakışmalarını düzelt - kademeli yaklaşım
            correction_strength = 0.8 - (iteration * 0.15)  # İterasyonla azalan düzeltme gücü
                ...
                for j in range(i + 2, n):  # Ardışık olmayan atomlar için
                    ...
                    if dist < min_allowed_dist:
                        ...
                        if motif_i == 'stem':
                            ...
                            weight_i = 0.8  # Terminal bölgeler en çok hareket eder
            ...
            # 2. Ardışık nükleotidler arası bağ uzunluğu kontrolü - İYİLEŞTİRİLMİŞ
            bond_correction_strength = 0.7 - (iteration * 0.1)  # İterasyonla azalan düzeltme gücü
            ...
            for i in range(n - 1):
                ...
                if abs(dist - ideal_bond_length) > 1.2:  # Tolerans 1.5'ten 1.2'ye azaltıldı
                    changes_made = True
            ...
            # 3. Watson-Crick baz çiftleri arası mesafeleri düzelt - YENİ ÖZELLİK
            if struct_idx > 2:  # Sadece yapı 3, 4 ve 5'te uygula (Yapı 2'yi daha serbest bırak)
                wc_correction_strength = 0.6 - (iteration * 0.1)
                ...
                for i, j in wc_pairs:
                    if j - i > 2:  # Gerçek W-C çiftleri en az 3 nükleotid aralıklı olmalı
                        ...
                        if (sequence[i] == 'G' and sequence[j] == 'C') or (sequence[i] == 'C' and sequence[j] == 'G'):
                            ideal_wc_dist = 10.5  # G-C için biraz daha uzun
                            ...
                            ideal_wc_dist = 10.3  # G-U wobble için orta mesafe

                        # Eğer mesafe çok farklıysa düzelt
                        if abs(dist - ideal_wc_dist) > 2.0:
                            changes_made = True
                            ...
                            if dist > ideal_wc_dist:
                                correction = (dist - ideal_wc_dist) * 0.3 * wc_correction_strength  # Yaklaştır
                            else:
                                correction = (ideal_wc_dist - dist) * 0.3 * wc_correction_strength  # Uzaklaştır
...
def process_prediction(prediction):
    ...
    # 2. İyileştirilmiş Biyolojik Filtreleme
...
print("İyileştirilmiş Biyolojik Filtreleme ile Termal Ensemble Kabsch stratejisi uygulanıyor...")
all_predictions = []
...
plt.tight_layout()
plt.savefig('improved_biological_filtering.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(
    f"\nİyileştirilmiş Biyolojik Filtreleme ile Termal Ensemble Kabsch Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")

# İyileştirmeleri özetle
print("\n--- İYİLEŞTİRME ÖZETİ ---")
print("1. Biyolojik Filtreleme - İyileştirilmiş versiyonu kullanıldı:")
print("   - Daha fazla iterasyon (3'ten 5'e)")
print("   - İterasyonla azalan düzeltme gücü (kademeli yaklaşım)")
print("   - Atom çakışmaları için daha hassas minimum mesafe (3.5 Å)")
print("   - Bağ uzunlukları için daha sıkı tolerans (1.5Å'dan 1.2Å'a)")
print("   - Watson-Crick baz çiftleri için mesafe düzeltmesi (YENİ ÖZELLİK)")
print("   - Baz çifti tipine göre özelleştirilmiş mesafeler (GC: 10.5Å, AU: 10.0Å, GU: 10.3Å)")
print("   - Motif-bazlı hareketlilik iyileştirmesi (terminal bölgeler için ek ağırlık: 0.8)")

print("\nBu iyileştirmeler, başarılı 0.199 skorlu stratejimizi daha ileri taşımak için Nature Methods makalesindeki")
print("biyolojik gerçeklik prensiplerini daha iyi uygulamayı hedeflemektedir.")