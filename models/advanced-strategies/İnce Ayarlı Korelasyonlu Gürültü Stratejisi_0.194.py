# RNA 3D Folding - İnce Ayarlı Korelasyonlu Gürültü Stratejisi
import pandas as pd
import numpy as np
...
print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (İnce Ayarlı Korelasyonlu Gürültü)...")
...
# 5. TAHMİN OLUŞTURMA VE İNCE AYRALI KORELASYONLU GÜRÜLTÜ STRATEJİSİ
...
# İNCE AYARLI KORELASYONLU GÜRÜLTÜ STRATEJİSİ
def generate_optimized_correlated_noise_strategy(prediction):
    """İnce ayarlı korelasyonlu gürültü kullanan geliştirilmiş varyasyon stratejisi"""
    result = []
    ...
    np.random.seed(126)  # İyileştirilmiş seed değeri (126)
    residue_factors = {}
    ...
    # Yapı 2: İnce ayarlı korelasyonlu gürültü (0.23 * 10)
    np.random.seed(126)
    noise_scale_2 = 0.23 * 10  # İnce ayar: 0.25 -> 0.23

    for entry in result:
        resid = entry['resid']
        rel_pos = resid / sequence_length

        # Adaptif faktörler - İyileştirilmiş
        # Baz tipi için daha hassas ayar
        if entry['resname'] == 'A':
            base_factor = 1.6
        elif entry['resname'] == 'U':
            base_factor = 1.4
        elif entry['resname'] == 'G':
            base_factor = 0.75
        else:  # C ve diğerleri
            base_factor = 0.85
        ...
        # Kademeli kenar faktörü
        if rel_pos < 0.10 or rel_pos > 0.90:
            edge_factor = 2.0
        elif rel_pos < 0.20 or rel_pos > 0.80:
            edge_factor = 1.5
        else:
            edge_factor = 1.0
        ...
        corr_weight = 0.72  # İnce ayar: 0.7 -> 0.72
        indiv_weight = 0.6  # Bireysel hareket ağırlığı aynı kaldı
    ...
    # Yapı 3: İnce ayarlı orta seviye korelasyonlu gürültü (0.52 * 10)
    np.random.seed(126)
    noise_scale_3 = 0.52 * 10  # İnce ayar: 0.50 -> 0.52
    ...
    for entry in result:
        ...
        corr_weight = 0.74  # İnce ayar: 0.7 -> 0.74
        indiv_weight = 0.6

        # Yapı 2'den hafif etkilenme - %15 oranında
        prev_dx = entry['x_2'] - entry['x_1']
        prev_dy = entry['y_2'] - entry['y_1']
        prev_dz = entry['z_2'] - entry['z_1']
        influence_factor = 0.15

        # Normal/Laplace karışımı dağılım
        if np.random.rand() < 0.75:  # %75 Normal
            random_noise_x = np.random.normal(0, noise * indiv_weight)
            random_noise_y = np.random.normal(0, noise * indiv_weight)
            random_noise_z = np.random.normal(0, noise * indiv_weight)
        else:  # %25 Laplace
            random_noise_x = np.random.laplace(0, noise * indiv_weight * 0.8)
            random_noise_y = np.random.laplace(0, noise * indiv_weight * 0.8)
            random_noise_z = np.random.laplace(0, noise * indiv_weight * 0.8)

        entry['x_3'] = entry['x_1'] + corr_factor * noise * corr_weight + random_noise_x + prev_dx * influence_factor
        entry['y_3'] = entry['y_1'] + corr_factor * noise * corr_weight + random_noise_y + prev_dy * influence_factor
        entry['z_3'] = entry['z_1'] + corr_factor * noise * corr_weight + random_noise_z + prev_dz * influence_factor

    # Yapı 4: İnce ayarlı yüksek seviye korelasyonlu gürültü (0.78 * 10)
    np.random.seed(126)
    noise_scale_4 = 0.78 * 10  # İnce ayar: 0.80 -> 0.78
    ...
    for entry in result:
        ...
        corr_weight = 0.77  # İnce ayar: 0.75 -> 0.77
        indiv_weight = 0.55
    ...
    # Yapı 5: İnce ayarlı ultra korelasyonlu gürültü + RNA uçlarına özel odak (0.72 * 10)
    np.random.seed(126)
    noise_scale_5 = 0.72 * 10  # İnce ayar: 0.70 -> 0.72
    ...
    for entry in result:
        ...
        # Kademeli kenar faktörü - daha agresif
        if rel_pos < 0.10 or rel_pos > 0.90:
            edge_factor = 3.2
        elif rel_pos < 0.20 or rel_pos > 0.80:
            edge_factor = 2.0
        ...
        corr_weight = 0.82  # İnce ayar: 0.8 -> 0.82
        indiv_weight = 0.5
...
# Her RNA için İnce Ayarlı Korelasyonlu Gürültü stratejisi uygula
print("İnce Ayarlı Korelasyonlu Gürültü stratejisi uygulanıyor...")
all_predictions = []

for prediction in tqdm(base_predictions, desc="RNA Yapıları İşleniyor"):
    structures = generate_optimized_correlated_noise_strategy(prediction)
...
strategies = [
    ...
    "Yapı 2: İnce Ayarlı Laplace (0.23*10)",
    "Yapı 3: İnce Ayarlı Karma Dağılım (0.52*10)",
    "Yapı 4: İnce Ayarlı Yüksek Gürültü (0.78*10)",
    "Yapı 5: İnce Ayarlı Ultra Korelasyon (0.72*10)"
]
...
plt.tight_layout()
plt.savefig('ince_ayarli_korelasyonlu_gurultu_strategy.png')
plt.close()

# Toplam çalışma süresi
end_time = time.time()
print(f"\nİnce Ayarlı Korelasyonlu Gürültü Pipeline tamamlandı! Toplam süre: {(end_time - start_time):.2f} saniye")