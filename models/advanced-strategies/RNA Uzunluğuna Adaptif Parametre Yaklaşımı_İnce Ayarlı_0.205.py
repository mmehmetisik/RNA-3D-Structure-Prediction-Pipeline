...
# 1. KONFİGÜRASYON PARAMETRELERİ - RNA UZUNLUĞUNA ADAPTİF YAKLAŞIM İÇİN İNCE AYAR
CONFIG = {
    ...
    # RNA Uzunluğuna göre Adaptif Parametreler - İNCE AYAR YAPILDI
    'length_params': {
        'short': {  # < 40 nükleotid için (eski: < 50)
            'min_allowed_dist': 3.32,  # Daha az sıkı çakışma kontrolü (3.30 -> 3.32)
            'bond_length_tolerance': 0.98,  # Daha dengeli bağ uzunluğu kontrolü (0.95 -> 0.98)
            'wc_distance_tolerance': 1.6,  # Daha dengeli Watson-Crick mesafesi (1.5 -> 1.6)
            'correction_strength_start': 0.87,  # Daha dengeli düzeltme (0.90 -> 0.87)
            'correction_strength_decay': 0.09,  # Aynı
            'num_iterations': 8,  # Daha dengeli iterasyon sayısı (9 -> 8)
            'non_wc_correction_strength': 0.38  # Daha dengeli non-WC düzeltmesi (0.40 -> 0.38)
        },
        'medium': {  # 40-120 nükleotid için (eski: 50-100)
            'min_allowed_dist': 3.35,  # Aynı
            'bond_length_tolerance': 1.05,  # Aynı
            'wc_distance_tolerance': 1.7,  # Aynı
            'correction_strength_start': 0.88,  # Aynı
            'correction_strength_decay': 0.11,  # Aynı
            'num_iterations': 7,  # Aynı
            'non_wc_correction_strength': 0.35  # Aynı
        },
        'long': {  # > 120 nükleotid için (eski: > 100)
            'min_allowed_dist': 3.38,  # Daha az esnek (3.40 -> 3.38)
            'bond_length_tolerance': 1.12,  # Daha az esnek (1.15 -> 1.12)
            'wc_distance_tolerance': 1.85,  # Daha az esnek (1.9 -> 1.85)
            'correction_strength_start': 0.82,  # Daha dengeli (0.80 -> 0.82)
            'correction_strength_decay': 0.12,  # Daha dengeli (0.13 -> 0.12)
            'num_iterations': 6,  # İterasyon artırıldı (5 -> 6)
            'non_wc_correction_strength': 0.32  # Daha dengeli (0.30 -> 0.32)
        }
    },
...
def identify_non_wc_pairs(sequence, sec_structure, motifs):
    """Watson-Crick olmayan potansiyel baz çiftlerini tanımlar"""
    non_wc_pairs = []
    n = len(sequence)
...
def generate_thermal_ensemble(prediction, norm_params, length_category, config=CONFIG):
    ...
    for i, entry in enumerate(result):
        ...
        if length_category == 'short':
            edge_factor *= 0.85  # Kısa RNA'lar için daha az gürültü (önceki 0.9 -> 0.85)
        elif length_category == 'long':
            edge_factor *= 1.15  # Uzun RNA'lar için daha fazla gürültü (önceki 1.1 -> 1.15)
    ...
    for i, entry in enumerate(result):
        ...
        # RNA uzunluk kategorisine göre faktör ayarla - İnce Ayar
        if length_category == 'short':
            edge_factor *= 0.85  # Kısa RNA'lar için daha az gürültü (önceki 0.8 -> 0.85)
        elif length_category == 'long':
            edge_factor *= 1.15  # Uzun RNA'lar için daha fazla gürültü (önceki 1.2 -> 1.15)
    ...
    for i, entry in enumerate(result):
        ...
        # RNA uzunluk kategorisine göre faktör ayarla - İnce Ayar
        if length_category == 'short':
            edge_factor *= 0.85  # Kısa RNA'lar için daha az gürültü (önceki 0.75 -> 0.85)
        elif length_category == 'long':
            edge_factor *= 1.15  # Uzun RNA'lar için daha fazla gürültü (önceki 1.25 -> 1.15)
...
def apply_advanced_biological_filtering(structures, prediction, length_category, config=CONFIG):
    """İyileştirilmiş biyolojik filtreleme, dolaşıklık önleme ve koaksiyel istifleme optimizasyonu uygular - RNA uzunluğuna göre adaptif"""
    # RNA bilgilerini al
    sequence = prediction['sequence']
    motifs = prediction['motifs']
    ...
    for struct_idx in range(2, 6):  # Yapı 2-5 için
        ...
        # 3. WATSON-CRICK OLMAYAN BAZ ÇİFTLERİNİ OPTİMİZE ET
        ...
        if struct_idx > 2 and non_wc_pairs:
            for pair in non_wc_pairs:
                ...
                if abs(dist - ideal_dist) > length_params['non_wc_distance_tolerance']:
                    ...
                    # Düzeltme miktarı - RNA uzunluğuna göre adaptif
                    correction_strength = length_params['non_wc_correction_strength']
...
def determine_length_category(sequence_length):
    """RNA uzunluğuna göre kategori belirler - İnce Ayar"""
    if sequence_length < 40:  # Önceki < 50
        return 'short'
    elif sequence_length <= 120:  # Önceki <= 100
        return 'medium'
    else:  # Önceki > 100
        return 'long'
...
def process_prediction(prediction, norm_params, config=CONFIG):
    """Verilen tahmini işleyerek optimize edilmiş sonuçlar üretir - RNA uzunluğuna göre adaptif parametreler kullanır"""
    ...
    # 2. Gelişmiş Biyolojik Filtreleme ve Koaksiyel İstifleme Optimizasyonu - RNA uzunluğuna adaptif
...
def visualize_results(predictions_df, sample_target):
    ...
    plt.tight_layout()
    plt.savefig('rna_uzunluk_adaptif_ince_ayarli.png')
    plt.close()
...
def main():
    ...
    print("RNA 3D Yapı Tahmini Pipeline Başlatılıyor (RNA Uzunluğuna Adaptif Parametre Yaklaşımı - İnce Ayarlı)...")
    ...
    # 4. Her RNA için Termal Ensemble ve Filtreleme Stratejisi - RNA uzunluğuna adaptif
    print("RNA Uzunluğuna Adaptif Parametre stratejisi uygulanıyor (İnce Ayarlı)...")
    all_predictions = []
    ...
    print("\n--- RNA UZUNLUĞUNA ADAPTİF PARAMETRE İYİLEŞTİRMESİ (İNCE AYARLI) ÖZETİ ---")
    print("1. İnce Ayarlanmış Kategori Sınırları:")
    print(f"   - Kısa RNA: <40 nükleotid (önceki <50)")
    print(f"   - Orta RNA: 40-120 nükleotid (önceki 50-100)")
    print(f"   - Uzun RNA: >120 nükleotid (önceki >100)")

    print("\n2. İnce Ayarlanmış Parametre Değerleri:")
    print(f"   - Kısa RNA min_allowed_dist: 3.32Å (önceki 3.30Å)")
    print(f"   - Uzun RNA min_allowed_dist: 3.38Å (önceki 3.40Å)")
    print(f"   - Kısa RNA için iterasyon sayısı: 8 (önceki 9)")
    print(f"   - Uzun RNA için iterasyon sayısı: 6 (önceki 5)")

    print("\n3. Daha Dengeli Termal Faktörler:")
    print("   - Kısa RNA için gürültü faktörü: 0.85 (önceki 0.8-0.9)")
    print("   - Uzun RNA için gürültü faktörü: 1.15 (önceki 1.2-1.25)")

    print("\n4. İnce Ayarlanmış Bağ Toleransları:")
    print(f"   - Kısa RNA için bond_length_tolerance: 0.98Å (önceki 0.95Å)")
    print(f"   - Uzun RNA için bond_length_tolerance: 1.12Å (önceki 1.15Å)")

    print("\nBu ince ayarlar, RNA uzunluğuna göre adaptif parametre yaklaşımının daha dengeli")
    print("bir şekilde uygulanmasını sağlayarak skoru artırmayı hedefliyor.")
