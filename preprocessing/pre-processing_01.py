...
# 4. Eğitim etiketlerini hazırla
# Filtrelenen eğitim dizileri için etiketleri al
filtered_train_ids = filtered_train_sequences['target_id'].tolist()
filtered_train_labels = train_labels[train_labels['ID'].str.split('_').str[0].isin(filtered_train_ids)]
...
def normalize_coordinates(labels_df, coord_columns):
    ...
    for col in coord_columns:
        ...
        normalized_df[col] = (labels_df[col] - col_min) / (col_max - col_min)

    return normalized_df, (col_min, col_max)
...
normalization_params = {
    ...
    'train_normalization_params': train_normalization_params
}
...
with open(os.path.join(output_dir, "processed", "normalization_params.json"), 'w') as f:
    json.dump(str(normalization_params), f)
